...
class HallocModel(nn.Module):
    ...
    def forward(
        ...
        visual_inputs = {k: v.to(embeddings.device) for k, v in visual_inputs.items()}
        ...
        proj_embeddings = self.proj(embeddings.float())  # (batch_size, max_seq_len, hidden_dim)

        out_embeddings = self.multimodal_encoder(
            inputs_embeds=proj_embeddings,  # (batch_size, max_seq_len, hidden_dim) = (64, 8, 768)
            attention_mask=attention_masks,  # (batch_size, max_seq_len) = (64, 8)
        ...
        out_embeddings = out_embeddings[:, :-1, :]
