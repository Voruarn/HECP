...
def evaluate(
    ...
    for batch in tqdm(dataloader):
        ...
        attention_masks = batch["attention_masks"].to(device) # (B, L)

        # is there any sample with all zeros in attention_masks?
        # if so, print the warning message
        for attention_mask in attention_masks:
            if torch.sum(attention_mask) == 0:
                logger.warning("[0] Attention mask with all zeros detected.")
        ...
        for attention_mask in attention_masks:
            if torch.sum(attention_mask) == 0:
                logger.warning("[1] Attention mask with all zeros detected.")
        ...
        # is there any sample with all zeros in attention_masks?
        # if so, print the warning message
        if torch.sum(attention_masks) == 0:
            logger.warning("[2] Attention mask with all zeros detected.")
