...
def main(
    ...
    test_labels = 1.0 - pad_to_max_len_label(test_labels)
    ...
    for fname in val_label_fnames[:25] + val_label_fnames[-25:]:
        labels = np.load(os.path.join(val_dir, fname))
        val_labels.extend(labels)
    
    val_labels = 1.0 - pad_to_max_len_label(val_labels)
    print(val_labels.shape)

    val_attention_masks = []

    for fname in val_attention_masks_fnames[:25] + val_attention_masks_fnames[-25:]:
    ...
    for fname in tqdm(val_logit_fnames[:25] + val_logit_fnames[-25:]):
    ...
    save_path = os.path.join(save_dir, f"internvl_calibration_original.json")
