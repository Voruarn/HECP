...
def postprocess(sample: Dict, tokenizer: AutoTokenizer) -> Dict:
    ...
    text = text.lower()
    tokenized_text = [tokenizer.decode(t) for t in tokens["input_ids"][0].tolist() if t not in [tokenizer.pad_token_id, tokenizer.bos_token_id, tokenizer.eos_token_id, 101, 102]]
            ...
            for h_list in h_type:
                for k in h_list.keys():
                    ...
                    try:
                        ...
                        print(h_type, h_list)
                        raise TypeError(f"Error: {hallucinated_answer['char_index']}")
                    ...
                    hallucinated_text = hallucinated_answer["name"].replace(" ", "").lower()
...
def main(
    ...
    output_path = input_path.replace(".json", "_text_postprocessed.json")
