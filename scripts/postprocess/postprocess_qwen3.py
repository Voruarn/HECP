...
def postprocess(sample: Dict, tokenizer: AutoTokenizer) -> Dict:
    ...
    tokenized_text = [tokenizer.decode(t) for t in tokens["input_ids"][0].tolist()
                      if t not in [tokenizer.pad_token_id, tokenizer.bos_token_id, tokenizer.eos_token_id]]
...
def main(
    input_path: str,
    tokenizer_name: str = "Qwen/Qwen3-VL-8B-Instruct",
) -> None:
    ...
    output_path = input_path.replace(".json", "_qwen3_postprocessed.json")
