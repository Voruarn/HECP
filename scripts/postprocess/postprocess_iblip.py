...
def init_tokenizer(tokenizer_name: str) -> LlamaTokenizer:
    ...
    llm_tokenizer.add_special_tokens({'bos_token': '</s>'}) # same with instructblip
    llm_tokenizer.add_special_tokens({'eos_token': '</s>'}) # same with instructblip
    llm_tokenizer.add_special_tokens({'unk_token': '</s>'}) # same with instructblip
...
def main(
    input_path: str,
    tokenizer_name:  str = "llm/vicuna-7b-v1.1",
) -> None:
    ...
    output_path = input_path.replace(".json", "_iblip_postprocessed.json")
