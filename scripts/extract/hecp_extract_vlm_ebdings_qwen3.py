...
# --- Qwen3-VL 前向传播，提取内部特征 + 回复起始位置 ---
def forward_qwen3vl_with_internals(model, processor, images, text_input, text_output, accelerator):
    device = accelerator.device
    batch_size = len(images)
    tokenizer = processor.tokenizer

    # 1. 构建 Qwen3-Instruct ChatML 格式的多模态对话
    conversations = []
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(test_dataloader)):
                ...
                all_hidden_states, all_attention_masks, all_input_ids, assistant_starts = \
                    forward_qwen3vl_with_internals(model, processor, images, text_input, text_output, accelerator)
